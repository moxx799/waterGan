...
import os
import pandas as pd
from Loader.dataset import CrackDatasetTF,get_train_augs,get_valid_augs,get_pred_augs,DisDatasetTF
...
def construct_dis_df(image_path, mask_path):
    image_files = os.listdir(image_path)
    mask_files = os.listdir(mask_path)

    #df = pd.DataFrame(columns=['images', 'labels'])
    
    # Label images from image_path as 0
    df1 = pd.concat([pd.DataFrame([(os.path.join(image_path, image_files[i]), 0)], columns=['images', 'labels']) for i in range(len(image_files))], ignore_index=True)
    
    # Label images from mask_path as 1
    df2 = pd.concat([pd.DataFrame([(os.path.join(mask_path, mask_files[i]), 1)], columns=['images', 'labels']) for i in range(len(mask_files))], ignore_index=True)
...
def config_dis_datasets(image_path, mask_path, val_images, val_files=None, batch_size=1):
    train_df = construct_dis_df(image_path, mask_path)
    train_set = DisDatasetTF(train_df)
    train_set = train_set.to_tf_dataset()
    train_dataset = train_set.batch(batch_size, drop_remainder=True).prefetch(tf.data.experimental.AUTOTUNE)
    
    val_df = construct_dis_df(val_images, val_files)
    val_set = DisDatasetTF(val_df)
    val_set = val_set.to_tf_dataset()
    val_dataset = val_set.batch(batch_size, drop_remainder=True).prefetch(tf.data.experimental.AUTOTUNE)
